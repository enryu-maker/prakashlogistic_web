        ...
        <div className='w-full flex flex-col items-center bg-white'>
            <div className="relative w-full h-[350px] flex items-center justify-center" style={{
                ...
                <div className="text-center text-white md:text-start w-full md:p-20">
                    <p className="text-4xl font-bold md:w-full md:text-start">Welcome to Our Company</p>
            ...
            <div className='flex w-[90%] lg:w-[78%] flex-col items-center my-16'>
                <p className='text-4xl font-semibold text-blue-gray-900 text-center'>OUR DELIVERY PARTNERS</p>
   
                <div className='flex flex-wrap w-full justify-evenly items-center mt-8'>
                    {
                        partner.map((item) => (
                            <div key={item.id} className=' rounded-full h-[150px] lg:h-[200px] w-[150px] lg:w-[200px] flex justify-center items-center m-4'>
                                <img src={item.icon} alt='' className='object-contain h-[100px] lg:h-[150px] w-[100px] lg:w-[150px]' />
                            </div>
                        ))
                    }
                </div>
                <p className='text-4xl font-semibold text-blue-gray-900 mt-16'>5 Years of <span className='text-primary tracking-widest'>Trust</span></p>
            </div>

            <div className='flex w-[90%] lg:w-[78%] flex-col items-center my-16'>
                <p className='text-4xl font-semibold text-blue-gray-900 text-center'>TRUSTED BY OUR 148+ CLIENTS</p>
                <Marquee gradient={true} gradientColor='white'> <div className='flex flex-wrap w-full justify-evenly items-center mt-8'>
                    {
                        clients.map((item) => (
                            <div key={item.id} className=' rounded-full h-[150px] lg:h-[200px] w-[150px] lg:w-[200px] flex justify-center items-center m-4'>
                                <img src={item.icon} alt='' className='object-contain h-[100px] lg:h-[150px] w-[100px] lg:w-[150px] rounded-full' />
                            </div>
                        ))
                    }
                </div></Marquee>
                <p className='text-4xl font-semibold text-blue-gray-900 mt-16 text-center'>And Many <span className='text-primary tracking-widest'>Moreeeeeeee</span></p>
            </div>
            <div className='w-full h-full'><Help /></div>
        </div>
    );
}
